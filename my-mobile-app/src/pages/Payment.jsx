...
export default function WhiteAndGreenRectangle() {
  ...
  const handleScroll = () => {
    ...
    if (isAtBottom) setShowBottomBar(false);
    else if (isScrollingUp) setShowBottomBar(true);
  ...
  return (
    <div style={{ width: "100%", minHeight: "100vh", position: "relative", backgroundColor: "#FFFFFF" }}>
      ...
      <div style={{ position: "absolute", top: vh(90), width: "100%", display: "flex", justifyContent: "center", gap: vw(90), zIndex: 2 }}>
        {["Menu", "Cart", "Checkout"].map((label, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <span style={{ marginTop: vw(4), fontSize: responsiveText(10), color: "#000000", fontFamily: "Poppins, sans-serif" }}>
              {label}
            </span>
          </div>
        ))}
      </div>
      <div style={{ position: "absolute", top: vh(110), width: "100%", height: vw(2), backgroundColor: "black", zIndex: 1 }} />

      <div
        ref={scrollContainerRef}
        onScroll={handleScroll}
        style={{ position: "absolute", top: vh(130), bottom: vh(10), width: "100%", overflowY: "auto", paddingBottom: vh(20) }}
      >
        {/* Payment Method */}
        <div style={{ width: "100%", padding: `0 ${vw(18)}`, boxSizing: "border-box", marginBottom: vh(25), marginTop: vh(40) }}>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(16), fontWeight: 600, marginBottom: vh(10), color: "#36570A" }}>
            Payment Method
          </p>
          <div style={{ border: `0.5px solid #CECECE`, borderRadius: vw(10), padding: `${vh(10)} ${vw(10)}`, backgroundColor: "#FFFFFF" }}>
            {["Cash", "G-Cash"].map((method) => (
              <div key={method}>
                <div onClick={() => setSelectedPayment(method)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", padding: `${vh(5)} 0` }}>
                  <span style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(14), color: "#000000" }}>{method}</span>
                  <CheckIconSVG isChecked={selectedPayment === method} style={{ width: vw(18), height: vw(18) }} />
                </div>
                {method === "Cash" && <div style={{ height: "0.5px", backgroundColor: "#E0E0E0", margin: `${vh(10)} 0` }} />}
              </div>
            ))}
          </div>
          <div style={{ marginTop: vh(10), backgroundColor: "#FFF8E7", padding: `${vh(8)} ${vw(12)}`, borderRadius: vw(8), border: "1px solid #E0C97F" }}>
            <p style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(10), fontWeight: 500, color: "#4B4B4B", margin: 0, lineHeight: 1.4 }}>
              💡 <strong>Note:</strong> For <strong>cash</strong>, please prepare the exact amount if possible. For <strong>GCash</strong>, have your phone ready for a quicker transaction.
            </p>
          </div>
        </div>

        {/* Order Summary */}
        <div style={{ width: "100%", padding: `0 ${vw(18)}`, boxSizing: "border-box", marginBottom: vh(25) }}>
          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(16), fontWeight: 600, color: "#36570A", marginBottom: vh(10) }}>
            Order Summary
          </p>
          <div>
            {mockOrderItems.map((item) => (
              <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: vh(5) }}>
                <span style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(12) }}>{item.quantity}x {item.name}</span>
                <span style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(12), fontWeight: 400, color: "black" }}>Php {item.price.toFixed(2)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Terms & Conditions */}
        <div style={{ width: "100%", padding: `0 ${vw(18)}`, boxSizing: "border-box" }}>
          <div style={{ border: "0.5px solid #CECECE", borderRadius: vw(10), padding: `${vh(10)} ${vw(10)}` }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: vw(8) }}>
              ...
              <p style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(9), fontWeight: 300, color: "#000000", margin: 0, lineHeight: 1.4 }}>
                <strong style={{ fontWeight: 500 }}>
                  I hereby give FASPecc USTP CDO Cafeteria the permission to share my customer data, including order history, contact information, and preferences, with authorized cafeteria personnel for processing, fulfilling, and improving my pre-orders.
                </strong>
              </p>
            </div>
          </div>

          <p style={{ fontFamily: "Poppins, sans-serif", fontSize: responsiveText(10), fontWeight: 500, textAlign: "left", marginTop: vh(10) }}>
            By completing this order, I agree to all{" "}
            <span onClick={handleTermsClick} style={{ color: "#36570A", textDecoration: "underline", cursor: "pointer", marginLeft: vw(3), marginRight: vw(3) }}>
              terms & conditions
            </span>.
          </p>
        </div>
